...
let resolveInputReader cmd =
  ...
  let inputFile =
    if IO.File.Exists givenInput then
      ...
      failwithf
        $"No input file found for command %s{cmd}, tried given input %s{givenInput} and fallback part 1 input %s{p1input}"
...
let runCommand cmd =
  ...
  let runMethod =
    ...
    match mi with
    | Some mi -> mi
    | None -> failwithf $"Unable to find `run` method taking single string argument in %s{moduleName} module"
